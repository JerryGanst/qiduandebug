...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=p?`${p[1].split("/").pop()}:${p[2]}`:"unknown",l={...s,location:u};return this.warn(e,`[HAPPY-PATH] ${t}`,l,n),o}},m=new W;var It={};function Rt(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,pe.fileURLToPath)(It.url))}var Nt=Rt(),N=A.get("CLAUDE_MEM_DATA_DIR"),y=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,de.homedir)(),".claude"),Yt=(0,b.join)(y,"plugins","marketplaces","thedotmack"),qt=(0,b.join)(N,"archives"),Kt=(0,b.join)(N,"logs"),Jt=(0,b.join)(N,"trash"),Qt=(0,b.join)(N,"backups"),zt=(0,b.join)(N,"modes"),Zt=(0,b.join)(N,"settings.json"),me=(0,b.join)(N,"claude-mem.db"),es=(0,b.join)(N,"vector-db"),ts=(0,b.join)(N,"observer-sessions"),ss=(0,b.join)(y,"settings.json"),rs=(0,b.join)(y,"commands"),ns=(0,b.join)(y,"CLAUDE.md");function ue(r){(0,ce.mkdirSync)(r,{recursive:!0})}function le(){return(0,b.join)(Nt,"..")}var _e=require("crypto");var Ct=3e4;function Ee(r,e,t){return(0,_e.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function Te(r,e,t){let s=t-Ct;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var $=class{db;constructor(e=me){e!==":memory:"&&ue(N),this.db=new ge.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn()}initializeSchema(){this.db.run(`
    ...
    `).run(e,t,s,n.toISOString(),o).lastInsertRowid}getUserPrompt(e,t){return this.db.prepare(`
      ...
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),c=new Date(a).toISOString(),p=Ee(e,s.title,s.narrative),u=Te(this.db,p,a);if(u)return{id:u.id,createdAtEpoch:u.created_at_epoch};let T=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,p,c,a);return{id:Number(T.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),c=new Date(a).toISOString(),u=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,c,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let c=a??Date.now(),p=new Date(c).toISOString();return this.db.transaction(()=>{let l=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let h=T.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,p,c);l.push(Number(h.lastInsertRowid))}let E=null;if(n){let h=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,p,c);E=Number(h.lastInsertRowid)}return{observationIds:l,summaryId:E,createdAtEpoch:c}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,c=0,p){let u=p??Date.now(),l=new Date(u).toISOString();return this.db.transaction(()=>{let E=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let S of s){let f=g.run(e,t,S.type,S.title,S.subtitle,JSON.stringify(S.facts),S.narrative,JSON.stringify(S.concepts),JSON.stringify(S.files_read),JSON.stringify(S.files_modified),a||null,c,l,u);E.push(Number(f.lastInsertRowid))}let h;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,c,l,u);h=Number(f.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(u,o),{observationIds:E,summaryId:h,createdAtEpoch:u}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",c=e.map(()=>"?").join(","),p=[...e],u=o?`WHERE id IN (${c}) AND project = ?`:`WHERE id IN (${c})`;return o&&p.push(o),this.db.prepare(`
      SELECT * FROM session_summaries
      ${u}
      ORDER BY created_at_epoch ${i}
      ${a}
    `).all(...p)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",c=e.map(()=>"?").join(","),p=[...e],u=o?"AND s.project = ?":"";return o&&p.push(o),this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.memory_session_id
      ...
      ${a}
    `).all(...p)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],c,p;if(e!==null){let _=`
...
`}function st(r,e,t,s){let n=[];return s?n.push(...Be(r)):n.push(...ve(r)),s?n.push(...He()):n.push(...Le()),s?n.push(...We()):n.push(...De()),s?n.push(...Ve()):n.push(...Me()),P(t)&&(s?n.push(...Ye(e,t)):n.push(...xe(e,t))),n}var ee=L(require("path"),1);function B(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return m.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r?.substring(0,50)},e),[]}}function nt(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function ot(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function it(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function rt(r,e){return ee.default.isAbsolute(r)?ee.default.relative(e,r):r}function at(r,e,t){let s=B(r);if(s.length>0)return rt(s[0],e);if(t){let n=B(t);if(n.length>0)return rt(n[0],e)}return"General"}function Lt(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=it(n);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function Dt(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?B(r.facts).join(`
`):null}function Mt(r,e,t,s,n,o){let i=[];o?i.push(...qe(r)):i.push(...ke(r));let a=null,c="",p=!1;for(let u of e)if(u.type==="summary"){p&&(i.push(""),p=!1,a=null,c="");let l=u.data,T=nt(l.displayTime);o?i.push(...ze(l,T)):i.push(...Fe(l,T))}else{let l=u.data,T=at(l.files_modified,n,l.files_read),E=ot(l.created_at),g=E!==c,h=g?E:"";c=E;let _=t.has(l.id);if(T!==a&&(p&&i.push(""),o?i.push(...Ke(T)):i.push(...Ue(T)),a=T,p=!0),_){let S=Dt(l,s);o?i.push(...Qe(l,E,g,S,s)):(p&&!o&&(i.push(""),p=!1),i.push(...$e(l,h,S,s)),a=null)}else o?i.push(Je(l,E,g,s)):i.push(we(l,h,s))}return p&&i.push(""),i}function dt(r,e,t,s,n){let o=[],i=Lt(r);for(let[a,c]of i)o.push(...Mt(a,c,e,t,s,n));return o}function ct(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function pt(r,e){let t=[];return e?(t.push(...k("Investigated",r.investigated,d.blue)),t.push(...k("Learned",r.learned,d.yellow)),t.push(...k("Completed",r.completed,d.green)),t.push(...k("Next Steps",r.next_steps,d.magenta))):(t.push(...x("Investigated",r.investigated)),t.push(...x("Learned",r.learned)),t.push(...x("Completed",r.completed)),t.push(...x("Next Steps",r.next_steps))),t}function mt(r,e){return e?Ze(r):Pe(r)}function ut(r,e,t){return!P(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?et(r.totalDiscoveryTokens,r.totalReadTokens):Xe(r.totalDiscoveryTokens,r.totalReadTokens)}var xt=lt.default.join((0,_t.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function kt(){try{return new $}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,Et.unlinkSync)(xt)}catch(e){m.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return m.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function Ut(r,e){return e?tt(r):je(r)}function wt(r,e,t,s,n,o,i){let a=[],c=K(e);a.push(...st(r,c,s,i));let p=t.slice(0,s.sessionCount),u=Ce(p,t),l=Z(e,u),T=Ae(e,s.fullObservationCount);a.push(...dt(l,T,s,n,i));let E=t[0],g=e[0];ct(s,E,g)&&a.push(...pt(E,i));let h=z(e,s,o,n);return a.push(...mt(h,i)),a.push(...ut(c,s,i)),a.join(`
`).trimEnd()}async function te(r,e=!1){let t=V(),s=r?.cwd??process.cwd(),n=Se(s),o=r?.projects||[n],i=kt();if(!i)return"";try{let a=o.length>1?Ne(i,o,t):J(i,n,t),c=o.length>1?Ie(i,o,t):Q(i,n,t);return a.length===0&&c.length===0?Ut(n,e):wt(n,a,c,t,s,r?.session_id,e)}finally{i.close()}}0&&(module.exports={generateContext});
